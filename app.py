    ...
    try:
        ...
        if os.path.exists(PLANTILLA_PDF):
            ...
            def insertar_folio_formateado():
                x_base, y, tamaño_fuente, _ = coords_ags["folio"]
                año_actual = datetime.now().year
...
async def get_nombre(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            ...
            "📸 Envíe la foto de su comprobante aquí mismo.\n"
            f"⚠️ Si no paga en 36 horas, el folio será eliminado automáticamente.\n\n"
            f"📋 Para generar otro permiso use /chuleta"
        )
...
async def login_get(request: Request):
    # ✅ FIX Starlette 0.36+: request va como 1er arg, fuera del dict
...
async def admin_folios_get(request: Request):
    ...
    try:
        ...
        print(f"[ADMIN_FOLIOS] ✅ {len(folios)} registros obtenidos")
        ...
        for f in folios:
            try:
                ...
                print(f"[ADMIN_FOLIOS] Error fecha: {fecha_err}")
                f['estado_calc'] = "ERROR"
    ...
    return templates.TemplateResponse(request, "admin_folios.html", {
        "folios": folios
    })
...
async def admin_tablas_get(request: Request):
    ...
    return templates.TemplateResponse(request, "admin_tablas.html", {
        "tablas": tablas
    })
...
async def editar_folio_get(request: Request, folio: str):
    ...
    try:
        ...
        return templates.TemplateResponse(request, "editar_folio.html", {
            "registro": registro
        })
    except Exception as e:
...
async def estado_folio(folio: str, request: Request):
    try:
        print(f"[CONSULTA] Buscando: {folio}")
        ...
        print(f"[CONSULTA] {'ENCONTRADO' if row else 'NO ENCONTRADO'}: {folio_limpio}")

        if not row:
            return templates.TemplateResponse(request, "resultado_consulta.html", {
                "folio": folio_limpio,
                "vigente": False,
                "marca": "", "linea": "", "anio": "",
                "serie": "", "motor": "", "color": "",
                "nombre": "", "expedicion": "", "vencimiento": "",
                "no_encontrado": True
            })
        ...
        fecha_exp_str = datetime.fromisoformat(row['fecha_expedicion']).strftime('%d/%m/%Y')
        ...
        return HTMLResponse(f"<h1>Error interno</h1><p>{str(e)}</p>", status_code=500)
...
async def root():
    ...
    <head>
        ...
        <style>
            ...
            h1 {{ color: #1a237e; margin-bottom: 8px; font-size: 26px; }}
            h2 {{ color: #283593; margin-bottom: 30px; font-size: 20px; font-weight: normal; }}
            .badge {{
        ...
        <div class="container">
            ...
            <div class="info">
                <ul style="padding-left: 20px;">
                    ...
                    <li><strong>Costo del permiso:</strong> ${PRECIO_PERMISO} MXN</li>
                    <li><strong>Tiempo límite de pago:</strong> 36 horas</li>
